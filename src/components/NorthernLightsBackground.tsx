...
float auroraLayer(vec2 uv, float layerIndex, float time) {
  ...
  float waveA = sin(x * 5.0 + time * 0.55 + layerIndex * 1.7) * 0.055 * u_length;
  float waveB = sin(x * 13.0 - time * 0.35 + layerIndex * 2.2) * 0.025 * u_length;
  float flowNoise = (fbm(vec2(x * 2.5 + layerIndex * 3.0, time * 0.12)) - 0.5) * 0.13 * u_length;

  float path = (0.12 + layerOffset) * u_length + waveA + waveB + flowNoise;
...
export default function NorthernLightsBackground({
  color1 = [0.486, 0.173, 1.0], // #7c2cff
  color2 = [0.059, 0.675, 0.796], // #0faccb
  color3 = [0.890, 0.678, 0.247], // #e3ad3f
  colorIntensity = 1.0,
  speed = 1.0,
  length = 1.0,
  width = 1.0,
  ...
  const propsRef = useRef({ color1, color2, color3, colorIntensity, speed, length, width, staticMode });

  useEffect(() => {
    propsRef.current = { color1, color2, color3, colorIntensity, speed, length, width, staticMode };
  }, [color1, color2, color3, colorIntensity, speed, length, width, staticMode]);
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.warn("WebGL2 not supported, northern lights will not render");
      return;
    }
    ...
    // Base color stays deep void
    gl.uniform3fv(uniforms.base, [0.020, 0.012, 0.039]); // #05030a
    ...
    const render = (now: number) => {
      ...
      gl.uniform1f(uniforms.motion, 1.0); // handled by accumulatedTime above
