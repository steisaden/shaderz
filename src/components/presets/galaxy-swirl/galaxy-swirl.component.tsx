...
/**
 * JSDoc documentation for GalaxySwirl
 * @purpose Renders an interactive, high-performance WebGL cosmic galaxy swirl.
 * @version 1.0.0
 */
...
export default function GalaxySwirl({
  ...
  useEffect(() => {
    ...
    const compileShader = (source: string, type: number) => {
      ...
      if (!webgl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    ...
    const uStarDensityLoc = gl.getUniformLocation(program, 'uStarDensity');
    const uArmCurvatureLoc = gl.getUniformLocation(program, 'uArmCurvature');
    const uStarSizeLoc = gl.getUniformLocation(program, 'uStarSize');
    ...
    const uEnableBloomLoc = gl.getUniformLocation(program, 'uEnableBloom');
    const uTwinkleSpeedLoc = gl.getUniformLocation(program, 'uTwinkleSpeed');
    const uStarTrailsLoc = gl.getUniformLocation(program, 'uStarTrails');
    const uSwirlTypeLoc = gl.getUniformLocation(program, 'uSwirlType');
    ...
    const render = (time: number) => {
      ...
      gl.uniform1f(uRotationSpeedLoc, props.rotationSpeed);
      gl.uniform1f(uStarDensityLoc, props.starDensity);
      gl.uniform1f(uArmCurvatureLoc, props.armCurvature);
      gl.uniform1f(uStarSizeLoc, props.starSize);
      gl.uniform3f(uColor1Loc, props.color1[0], props.color1[1], props.color1[2]);
      gl.uniform3f(uColor2Loc, props.color2[0], props.color2[1], props.color2[2]);
      gl.uniform3f(uColor3Loc, props.color3[0], props.color3[1], props.color3[2]);
      gl.uniform1f(uColorIntensityLoc, props.colorIntensity);
      gl.uniform1i(uEnableBloomLoc, props.enableBloom ? 1 : 0);
      gl.uniform1f(uTwinkleSpeedLoc, props.twinkleSpeed);
      gl.uniform1f(uStarTrailsLoc, props.starTrails);
      gl.uniform1f(uSwirlTypeLoc, props.swirlType);
